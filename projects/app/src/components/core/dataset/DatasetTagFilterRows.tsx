...
/** 旧版本升级按钮，贴在标题栏右侧。 */
export const DatasetTagFilterUpgradeButton = ({
  onUpgrade
}: {
  ...
  const { runAsync: runUpgrade, loading } = useRequest(onUpgrade);
  ...
  return (
    <>
      <Box
        ...
        onClick={() => openConfirm({ onConfirm: runUpgrade })()}
      >
        {t('workflow:tag_filter_upgrade_cta')}
      </Box>
      <ConfirmModal isLoading={loading} />
    </>
  );
};
